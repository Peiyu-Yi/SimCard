...
# euclidean 距离
def calculate_distance_matrix(dataset, queries):
...
if __name__ == "__main__":
    ...
    train_dataset = np.load('/research/remote/petabyte/users/s3852583/dataset4simcard/face_trainingData.npy')
    test_dataset = np.load('/research/remote/petabyte/users/s3852583/dataset4simcard/face_testingData.npy')
    print(f"train_dataset size: {sys.getsizeof(train_dataset)}")
    print(f"test_dataset size: {sys.getsizeof(test_dataset)}")

    train_dictionary = generate_dictionary(train_dataset, 128)
    test_dictionary = generate_dictionary(test_dataset, 128)
    print(f"train_dictionary size: {sys.getsizeof(train_dictionary)}")
    print(f"test_dictionary size: {sys.getsizeof(test_dictionary)}")

    start_time = time.time()
    with open('/research/remote/petabyte/users/peiyu/SimCard/dataset/face_128_cosine/train_dictionary.pkl',
              'wb') as f:
    ...
    start_time = time.time()
    with open('/research/remote/petabyte/users/peiyu/SimCard/dataset/face_128_cosine/test_dictionary.pkl',
              'wb') as f:
